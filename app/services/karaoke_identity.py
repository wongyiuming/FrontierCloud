"""Opaque browser handles for owner-bound karaoke media identities."""
...
def issue(*, media_id: str | None = None, resource_id: str | None = None) -> str:
    ...
    kind, identifier = ("remote", resource_id) if resource_id else ("local", media_id)
...
def resolve(token: str) -> tuple[str, str]:
    ...
    if payload.get("v") != 1 or kind not in {"local", "remote"}:
