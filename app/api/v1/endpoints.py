...
def _without_paths(source: APIRouter, paths: set[str]) -> APIRouter:
    filtered = APIRouter()
    filtered.routes.extend(route for route in source.routes if route.path not in paths)
    return filtered
...
router.include_router(cluster_admin_router, prefix="/media/admin", tags=["MediaAdmin"])
