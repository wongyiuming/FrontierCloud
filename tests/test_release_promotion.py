...
class ReleasePromotionTests(unittest.TestCase):
    def run(self, *, tree: str, number: int, status: str = "completed", conclusion: str | None = "success"):
        return {
            "head_branch": "dev",
            "event": "push",
    ...
    def test_newest_matching_tree_is_authoritative(self):
        tree = "a" * 40
        runs = [
            self.run(tree=tree, number=9, conclusion="failure"),
            self.run(tree=tree, number=8, conclusion="success"),
        ]
    ...
    def test_unrelated_newer_dev_tree_is_ignored(self):
        target = "b" * 40
        runs = [
            self.run(tree="c" * 40, number=11),
            self.run(tree=target, number=10),
        ]
    ...
    def test_non_dev_or_non_push_run_cannot_promote(self):
        tree = "d" * 40
        wrong_branch = self.run(tree=tree, number=4)
        wrong_branch["head_branch"] = "main"
        wrong_event = self.run(tree=tree, number=3)
