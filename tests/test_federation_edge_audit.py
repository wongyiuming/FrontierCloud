...
class CapabilityAuditTests(unittest.IsolatedAsyncioTestCase):
    async def test_owner_uses_signed_provenance_even_when_public_headers_disagree(self):
        ...
        token = p.media_token(credential, relation_id, master_id, owner_id, original, media_id, int(time.time()),
        ...
        store = SimpleNamespace(node={"role": "Follower", "node_id": owner_id}, unseal=lambda value: value,
        ...
        self.assertEqual(response.headers["X-Media-Resource-ID"], media_id)
    ...
    def test_capability_provenance_is_authenticated(self):
        credential, now = secrets.token_urlsafe(48), int(time.time())
        plain = p.media_token(
            credential, "a" * 32, "b" * 32, "c" * 32, "d" * 64, "9" * 64, now,
        )
        self.assertNotIn("request_id", p.verify_media_token(credential, plain, now))
        provenance = p.media_token(credential, "a" * 32, "b" * 32, "c" * 32, "d" * 64,
                                   "9" * 64, now,
