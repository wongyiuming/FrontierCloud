...
class PlayerUIContractTests(unittest.TestCase):
    ...
    def test_public_home_hides_elevation_and_refresh_behind_logo_hotspots(self):
        ...
        self.assertEqual(template.count('class="brand-surface card"'), 2)
        self.assertGreaterEqual(template.count('class="brand-backdrop"'), 3)
        self.assertGreaterEqual(template.count('class="brand-foreground"'), 3)
        self.assertIn("overflow:hidden", template)
        self.assertIn("border-radius:clamp(", template)
